...
```typescript
// Configuration options (PAT can be passed directly or fall back to env var)
interface Config {
  organization?: string;  // Falls back to ADO_ORGANIZATION env var
  pat?: string;           // Falls back to ADO_PAT env var
}

// Option 1: Pass PAT directly (recommended for multi-tenant or dynamic scenarios)
const config: Config = {
  organization: 'my-org',
  pat: 'my-personal-access-token',
};

// Option 2: Use environment variables as fallback
const organization = config.organization || process.env.ADO_ORGANIZATION;
const pat = config.pat || process.env.ADO_PAT;

if (!organization || !pat) {
  throw new Error('Organization and PAT are required (via config or env vars)');
}
...
The Azure DevOps client supports two authentication methods:

1. **Direct Parameter** (Recommended for multi-tenant/dynamic scenarios)
   - Pass `pat` directly in the configuration object
   - Useful when working with multiple organizations or rotating tokens

2. **Environment Variable Fallback**
   - If no `pat` is provided, falls back to `ADO_PAT` environment variable
   - Ideal for single-tenant setups or CI/CD pipelines

### Configuration Priority

| Parameter | Direct Config | Environment Fallback | Required |
|-----------|---------------|---------------------|----------|
| `organization` | `config.organization` | `ADO_ORGANIZATION` | Yes |
| `pat` | `config.pat` | `ADO_PAT` | Yes |

**Note**: Direct configuration takes precedence over environment variables.
...
```typescript
/**
 * Creates authentication header for Azure DevOps API
 * @param pat - Personal Access Token (optional, falls back to ADO_PAT env var)
 * @returns Basic auth header string
 */
function createAuthHeader(pat?: string): string {
  const token = pat || process.env.ADO_PAT;
  if (!token) {
    throw new Error('PAT is required: pass as parameter or set ADO_PAT env var');
  }
...
// Usage examples:
// createAuthHeader('my-custom-pat')      // Uses provided PAT
// createAuthHeader()                      // Uses ADO_PAT env var
```
...
interface AzureDevOpsClientConfig {
  organization?: string;      // Optional: falls back to ADO_ORGANIZATION env var
  pat?: string;               // Optional: falls back to ADO_PAT env var
...
// Example: Using direct PAT (multi-tenant scenario)
const clientWithPat = new AzureDevOpsClient({
  organization: 'tenant-a-org',
  pat: 'tenant-a-token',
});

// Example: Using environment variables (single-tenant scenario)
const clientWithEnv = new AzureDevOpsClient();  // Uses ADO_ORGANIZATION & ADO_PAT
```
...
> **Note**: The standalone function examples below use a global `httpClient`. For flexible authentication with optional PAT parameters, see the [TypeScript Client Pattern](#typescript-client-pattern) section which wraps all operations in a class that accepts `pat` as an optional parameter.
...
> **Note**: The standalone function examples below use a global `httpClient`. For flexible authentication with optional PAT parameters, see the [TypeScript Client Pattern](#typescript-client-pattern) section.
...
This is the **recommended approach** for interacting with Azure DevOps APIs. The client class:

- **Accepts optional `pat` parameter** - pass directly or fall back to `ADO_PAT` env var
- **Accepts optional `organization` parameter** - pass directly or fall back to `ADO_ORGANIZATION` env var
- Provides built-in retry logic with exponential backoff
- Handles authentication header generation automatically
- Wraps all API operations in a clean, typed interface
 ...
 * All properties are optional - they fall back to environment variables if not provided.
 */
...
// Usage Example 1: Using environment variables (single-tenant)
// Set ADO_ORGANIZATION and ADO_PAT env vars
const clientWithEnv = new AzureDevOpsClient();
const workItem1 = await clientWithEnv.getWorkItem('MyProject', 12345);

// Usage Example 2: Using direct PAT (multi-tenant or dynamic scenarios)
const clientWithPat = new AzureDevOpsClient({
  organization: 'tenant-a-org',
  pat: 'tenant-a-token',
});
const workItem2 = await clientWithPat.getWorkItem('TenantAProject', 67890);

// Usage Example 3: Mix - organization from config, PAT from env
const clientMixed = new AzureDevOpsClient({
  organization: 'specific-org',  // Override organization
  // pat not provided - falls back to ADO_PAT env var
});
```
