...
def main():
  ...
  # Define valid commands for validation (before client instantiation)
  valid_commands = {
    'get-work-item', 'get-project', 'list-projects', 'query',
    'list-repos', 'list-prs', 'list-pipelines', 'list-teams',
    'list-branches', 'list-commits', 'get-pr', 'create-work-item',
    'update-work-item', 'add-comment'
  }

  # Handle help before client creation (no credentials needed)
  if command in ('help', '--help', '-h'):
    print(__doc__)
    sys.exit(0)

  # Validate command before creating client
  if command not in valid_commands:
    print(f"Unknown command: {command}")
    print(f"Available commands: {', '.join(sorted(valid_commands))}")
    sys.exit(1)

  # Only create client after confirming valid command
